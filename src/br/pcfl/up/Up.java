...
import br.pcfl.up.activity.MessageCompose;
import br.pcfl.up.activity.UpgradeDatabases;
...
import br.pcfl.up.mail.internet.BinaryTempFileBody;
import br.pcfl.up.mail.store.LocalStore;
import br.pcfl.up.provider.UnreadWidgetProvider;
import br.pcfl.up.security.LocalKeyStore;
import br.pcfl.up.service.BootReceiver;
import br.pcfl.up.service.MailService;
import br.pcfl.up.service.ShutdownReceiver;
import br.pcfl.up.service.StorageGoneReceiver;


public class Up extends Application {
    /**
     * Components that are interested in knowing when the Up instance is
     * available and ready (Android invokes Application.onCreate() after other
     * components') should implement this interface and register using
     * {@link Up#registerApplicationAware(ApplicationAware)}.
     */
    ...
    public static final String LOG_TAG = "Up";
     ...
     * was called.
     * Afterwards calls to {@link #registerApplicationAware(br.pcfl.up.Up.ApplicationAware)} will
     * immediately call {@link br.pcfl.up.Up.ApplicationAware#initializeComponent(Up)} for the
     * supplied argument.
     */
    ...
    //public static final String logFile = Environment.getExternalStorageDirectory() + "/Upmail/debug.log";
    ...
    /**
     * Should Up log the conversation it has over the wire with
     * SMTP servers?
     */
     ...
     */
    public static String ERROR_FOLDER_NAME = "Errors Log";
    ...
    public static final String LOCAL_UID_PREFIX = "UpLOCAL:";

    public static final String REMOTE_UID_PREFIX = "UpREMOTE:";

    public static final String IDENTITY_HEADER = "X-Upmail-Identity";
    ...
    /* How many times should up try to deliver a message before giving up
    ...
    public static class Intents {
        ...
        public static class EmailReceived {
            public static final String ACTION_EMAIL_RECEIVED    = "br.pcfl.up.intent.action.EMAIL_RECEIVED";
            public static final String ACTION_EMAIL_DELETED     = "br.pcfl.up.intent.action.EMAIL_DELETED";
            public static final String ACTION_REFRESH_OBSERVER  = "br.pcfl.up.intent.action.REFRESH_OBSERVER";
            public static final String EXTRA_ACCOUNT            = "br.pcfl.up.intent.extra.ACCOUNT";
            public static final String EXTRA_FOLDER             = "br.pcfl.up.intent.extra.FOLDER";
            public static final String EXTRA_SENT_DATE          = "br.pcfl.up.intent.extra.SENT_DATE";
            public static final String EXTRA_FROM               = "br.pcfl.up.intent.extra.FROM";
            public static final String EXTRA_TO                 = "br.pcfl.up.intent.extra.TO";
            public static final String EXTRA_CC                 = "br.pcfl.up.intent.extra.CC";
            public static final String EXTRA_BCC                = "br.pcfl.up.intent.extra.BCC";
            public static final String EXTRA_SUBJECT            = "br.pcfl.up.intent.extra.SUBJECT";
            public static final String EXTRA_FROM_SELF          = "br.pcfl.up.intent.extra.FROM_SELF";
        }

        public static class Share {
            /*
             * We don't want to use EmailReceived.EXTRA_FROM ("br.pcfl.up.intent.extra.FROM")
             * because of different semantics (String array vs. string with comma separated
             * email addresses)
             */
            public static final String EXTRA_FROM               = "br.pcfl.up.intent.extra.SENDER";
        }
    }
    ...
    protected void registerReceivers() {
        ...
        try {
            ...
            Log.i(Up.LOG_TAG, "Registered: unmount receiver");
        } catch (InterruptedException e) {
            Log.e(Up.LOG_TAG, "Unable to register unmount receiver", e);
        }

        registerReceiver(new ShutdownReceiver(), new IntentFilter(Intent.ACTION_SHUTDOWN));
        Log.i(Up.LOG_TAG, "Registered: shutdown receiver");
    }

    public static void save(SharedPreferences.Editor editor) {
        editor.putBoolean("enableDebugLogging", Up.DEBUG);
        editor.putBoolean("enableSensitiveLogging", Up.DEBUG_SENSITIVE);
        editor.putString("backgroundOperations", Up.backgroundOps.toString());
        ...
        MessagingController.getInstance(this).addListener(new MessagingListener() {
            private void broadcastIntent(String action, Account account, String folder, Message message) {
                try {
                    ...
                    intent.putExtra(Up.Intents.EmailReceived.EXTRA_ACCOUNT, account.getDescription());
                    intent.putExtra(Up.Intents.EmailReceived.EXTRA_FOLDER, folder);
                    intent.putExtra(Up.Intents.EmailReceived.EXTRA_SENT_DATE, message.getSentDate());
                    intent.putExtra(Up.Intents.EmailReceived.EXTRA_FROM, Address.toString(message.getFrom()));
                    intent.putExtra(Up.Intents.EmailReceived.EXTRA_TO, Address.toString(message.getRecipients(Message.RecipientType.TO)));
                    intent.putExtra(Up.Intents.EmailReceived.EXTRA_CC, Address.toString(message.getRecipients(Message.RecipientType.CC)));
                    intent.putExtra(Up.Intents.EmailReceived.EXTRA_BCC, Address.toString(message.getRecipients(Message.RecipientType.BCC)));
                    intent.putExtra(Up.Intents.EmailReceived.EXTRA_SUBJECT, message.getSubject());
                    intent.putExtra(Up.Intents.EmailReceived.EXTRA_FROM_SELF, account.isAnIdentity(message.getFrom()));
                    Up.this.sendBroadcast(intent);
                    if (Up.DEBUG)
                        Log.d(Up.LOG_TAG, "Broadcasted: action=" + action
                    ...
                    Log.w(Up.LOG_TAG, "Error: action=" + action
                          + " account=" + account.getDescription()
                          + " folder=" + folder
                          + " message uid=" + message.getUid()
                         );
            ...
            private void updateUnreadWidget() {
                try {
                    UnreadWidgetProvider.updateUnreadCount(Up.this);
                } catch (Exception e) {
                    if (Up.DEBUG) {
            ...
            public void synchronizeMailboxRemovedMessage(Account account, String folder, Message message) {
                broadcastIntent(Up.Intents.EmailReceived.ACTION_EMAIL_DELETED, account, folder, message);
                updateUnreadWidget();
            }
            ...
            public void synchronizeMailboxNewMessage(Account account, String folder, Message message) {
                broadcastIntent(Up.Intents.EmailReceived.ACTION_EMAIL_RECEIVED, account, folder, message);
                updateUnreadWidget();
            }
            ...
            public void folderStatusChanged(Account account, String folderName,
                ...
                Intent intent = new Intent(Up.Intents.EmailReceived.ACTION_REFRESH_OBSERVER, null);
                intent.putExtra(Up.Intents.EmailReceived.EXTRA_ACCOUNT, account.getDescription());
                intent.putExtra(Up.Intents.EmailReceived.EXTRA_FOLDER, folderName);
                Up.this.sendBroadcast(intent);
    ...
    public static void loadPrefs(Preferences prefs) {
        ...
        if (!DEBUG && sIsDebuggable && Debug.isDebuggerConnected()) {
            ...
            DEBUG = true;
            Log.i(Up.LOG_TAG, "Debugger attached; enabling debug logging.");
        }
        ...
        useGalleryBugWorkaround = sprefs.getBoolean("useGalleryBugWorkaround", Up.isGalleryBuggy());
        ...
        Up.setUpLanguage(sprefs.getString("language", ""));
        ...
        if (themeValue == Theme.DARK.ordinal() || themeValue == android.R.style.Theme) {
            Up.setUpTheme(Theme.DARK);
        } else {
            Up.setUpTheme(Theme.LIGHT);
        }

        themeValue = sprefs.getInt("messageViewTheme", Theme.USE_GLOBAL.ordinal());
        Up.setUpMessageViewThemeSetting(Theme.values()[themeValue]);
        themeValue = sprefs.getInt("messageComposeTheme", Theme.USE_GLOBAL.ordinal());
        Up.setUpComposerThemeSetting(Theme.values()[themeValue]);
        Up.setUseFixedMessageViewTheme(sprefs.getBoolean("fixedMessageViewTheme", true));
    }
    ...
    protected void notifyObservers() {
        synchronized (observers) {
            for (final ApplicationAware aware : observers) {
                if (Up.DEBUG) {
                    Log.v(Up.LOG_TAG, "Initializing observer: " + aware);
                }
                try {
                    aware.initializeComponent(this);
                } catch (Exception e) {
                    Log.w(Up.LOG_TAG, "Failure when notifying " + aware, e);
                }
            }
    ...
    /**
     * Register a component to be notified when the {@link Up} instance is ready.
     *
     * @param component
    ...
    public static void registerApplicationAware(final ApplicationAware component) {
        synchronized (observers) {
            if (sInitialized) {
                component.initializeComponent(Up.app);
    ...
    public static String getUpLanguage() {
        return language;
    }

    public static void setUpLanguage(String nlanguage) {
        language = nlanguage;
    }
    ...
    public static int getUpThemeResourceId() {
        return getUpThemeResourceId(theme);
    }

    public static Theme getUpMessageViewTheme() {
        return messageViewTheme == Theme.USE_GLOBAL ? theme : messageViewTheme;
    }

    public static Theme getUpMessageViewThemeSetting() {
        return messageViewTheme;
    }

    public static Theme getUpComposerTheme() {
        return composerTheme == Theme.USE_GLOBAL ? theme : composerTheme;
    }

    public static Theme getUpComposerThemeSetting() {
        return composerTheme;
    }

    public static Theme getUpTheme() {
        return theme;
    }

    public static void setUpTheme(Theme ntheme) {
        if (ntheme != Theme.USE_GLOBAL) {
            theme = ntheme;
        }
    }

    public static void setUpMessageViewThemeSetting(Theme nMessageViewTheme) {
    ...
    public static void setUpComposerThemeSetting(Theme compTheme) {
    ...
    public static boolean setBackgroundOps(BACKGROUND_OPS backgroundOps) {
        BACKGROUND_OPS oldBackgroundOps = Up.backgroundOps;
        Up.backgroundOps = backgroundOps;
    ...
    public static void setUseGalleryBugWorkaround(boolean useGalleryBugWorkaround) {
        Up.useGalleryBugWorkaround = useGalleryBugWorkaround;
    }
    ...
    public static void setAttachmentDefaultPath(String attachmentDefaultPath) {
        Up.mAttachmentDefaultPath = attachmentDefaultPath;
    }
