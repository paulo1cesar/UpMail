...
public class UpRemoteControl {
    ...
    public static void set(Context context, Intent broadcastIntent) {
        broadcastIntent.setAction(UpRemoteControl.K9_SET);
        context.sendBroadcast(broadcastIntent, UpRemoteControl.K9_REMOTE_CONTROL_PERMISSION);
    }

    public static void fetchAccounts(Context context, UpAccountReceptor receptor) {
        Intent accountFetchIntent = new Intent();
        accountFetchIntent.setAction(UpRemoteControl.K9_REQUEST_ACCOUNTS);
        AccountReceiver receiver = new AccountReceiver(receptor);
        context.sendOrderedBroadcast(accountFetchIntent, UpRemoteControl.K9_REMOTE_CONTROL_PERMISSION, receiver, null, Activity.RESULT_OK, null, null);
    }
