...
import br.pcfl.up.controller.MessageRetrievalListener;
import br.pcfl.up.helper.StringUtils;
import br.pcfl.up.helper.Utility;
...
import br.pcfl.up.mail.Authentication;
import br.pcfl.up.mail.AuthenticationFailedException;
import br.pcfl.up.mail.Body;
...
import br.pcfl.up.mail.filter.Base64;
import br.pcfl.up.mail.filter.EOLConvertingOutputStream;
import br.pcfl.up.mail.filter.FixedLengthInputStream;
import br.pcfl.up.mail.filter.PeekableInputStream;
import br.pcfl.up.mail.internet.MimeBodyPart;
import br.pcfl.up.mail.internet.MimeHeader;
import br.pcfl.up.mail.internet.MimeMessage;
import br.pcfl.up.mail.internet.MimeMultipart;
import br.pcfl.up.mail.internet.MimeUtility;
import br.pcfl.up.mail.store.ImapResponseParser.ImapList;
import br.pcfl.up.mail.store.ImapResponseParser.ImapResponse;
import br.pcfl.up.mail.store.imap.ImapUtility;
import br.pcfl.up.mail.transport.imap.ImapSettings;
import br.pcfl.up.net.ssl.TrustManagerFactory;
import br.pcfl.up.net.ssl.TrustedSocketFactory;
...
public class ImapStore extends Store {
        ...
        for (ImapResponse response : responses) {
            if (ImapResponseParser.equalsIgnoreCase(response.get(0), commandResponse)) {
                ...
                if (response.size() > 4 || !(response.getObject(3) instanceof String)) {
                    Log.w(Up.LOG_TAG, "Skipping incorrectly parsed " + commandResponse +
                            " reply: " + response);
                    continue;
                }
                ...
                try {
                    ...
                    Log.w(Up.LOG_TAG, "Folder name not correctly encoded with the UTF-7 variant " +
     ...
     * {@link #listFolders(br.pcfl.up.mail.store.ImapStore.ImapConnection, boolean)}; we should try to consolidate
     * this at some point. :(
    ...
    private void autoconfigureFolders(final ImapConnection connection) throws IOException, MessagingException {
        ...
        if (connection.capabilities.contains("XLIST")) {
            if (Up.DEBUG) Log.d(Up.LOG_TAG, "Folder auto-configuration: Using XLIST.");
            commandResponse = "XLIST";
        } else if(connection.capabilities.contains("SPECIAL-USE")) {
            if (Up.DEBUG) Log.d(Up.LOG_TAG, "Folder auto-configuration: Using RFC6154/SPECIAL-USE.");
            commandResponse = "LIST";
            commandOptions = " (SPECIAL-USE)";
        } else {
            if (Up.DEBUG) Log.d(Up.LOG_TAG, "No detected folder auto-configuration methods.");
            return;
        }
            ...
            if (ImapResponseParser.equalsIgnoreCase(response.get(0), commandResponse)) {
                ...
                for (int i = 0, count = attributes.size(); i < count; i++) {
                    ...
                    if (attribute.equals("\\Drafts")) {
                        ...
                        if (Up.DEBUG) Log.d(Up.LOG_TAG, "Folder auto-configuration detected draft folder: " + decodedFolderName);
                    } else if (attribute.equals("\\Sent")) {
                        mAccount.setSentFolderName(decodedFolderName);
                        if (Up.DEBUG) Log.d(Up.LOG_TAG, "Folder auto-configuration detected sent folder: " + decodedFolderName);
                        ...
                        if (Up.DEBUG) Log.d(Up.LOG_TAG, "Folder auto-configuration detected spam folder: " + decodedFolderName);
                    } else if (attribute.equals("\\Trash")) {
                        mAccount.setTrashFolderName(decodedFolderName);
                        if (Up.DEBUG) Log.d(Up.LOG_TAG, "Folder auto-configuration detected trash folder: " + decodedFolderName);
                    }
                }
            }
        }
    ...
    public void checkSettings() throws MessagingException {
        try {
            ...
            throw new MessagingException(Up.app.getString(R.string.error_unable_to_connect), ioe);
        }
    }
    ...
    class ImapFolder extends Folder {
        ...
        public List<ImapResponse> internalOpen(int mode) throws MessagingException {
            ...
            try {
                ...
                Log.e(Up.LOG_TAG, "Unable to open connection for " + getLogId(), me);
                throw me;
            }
        }
        ...
        public void close() {
            ...
            synchronized (this) {
                ...
                if (mInSearch && mConnection != null) {
                    Log.i(Up.LOG_TAG, "IMAP search was aborted, shutting down connection.");
                    mConnection.close();
                } else {
                    releaseConnection(mConnection);
                }
        ...
        public Map<String, String> copyMessages(Message[] messages, Folder folder)
            ...
            try {
                ...
                if (!exists(remoteDestName)) {
                     ...
                     */
                    if (Up.DEBUG) {
                        Log.i(Up.LOG_TAG, "ImapFolder.copyMessages: attempting to create remote " +
                        ...
                        if (copyList.size() >= 4 && copyList.getString(0).equals("COPYUID")) {
                            ...
                            if (srcUids != null && destUids != null) {
                                if (srcUids.size() == destUids.size()) {
                                    ...
                                    if (Up.DEBUG) {
                                        Log.v(Up.LOG_TAG, "Parse error: size of source UIDs " +
                                ...
                                if (Up.DEBUG) {
                                    Log.v(Up.LOG_TAG, "Parsing of the sequence set failed.");
                                }
                            }
                        }
                    }
        ...
        public void delete(Message[] messages, String trashFolderName) throws MessagingException {
            ...
            if (trashFolderName == null || getName().equalsIgnoreCase(trashFolderName)) {
                ...
                if (!exists(remoteTrashName)) {
                     ...
                     */
                    if (Up.DEBUG)
                        Log.i(Up.LOG_TAG, "IMAPMessage.delete: attempting to create remote '" + trashFolderName + "' folder for " + getLogId());
                    remoteTrashFolder.create(FolderType.HOLDS_MESSAGES);
                }

                if (exists(remoteTrashName)) {
                    if (Up.DEBUG)
                        Log.d(Up.LOG_TAG, "IMAPMessage.delete: copying remote " + messages.length + " messages to '" + trashFolderName + "' for " + getLogId());
        ...
        protected long getHighestUid() {
            try {
                ...
                Log.e(Up.LOG_TAG, "Unable to find highest UID in folder " + getName(), e);
            }
            return -1L;
        ...
        throws MessagingException {
            ...
            if (fp.contains(FetchProfile.Item.ENVELOPE)) {
                ...
                fetchFields.add("BODY.PEEK[HEADER.FIELDS (date subject from content-type to cc " +
                        "reply-to message-id references in-reply-to " + Up.IDENTITY_HEADER + ")]");
            }
                        ...
                        if (response.mTag == null && ImapResponseParser.equalsIgnoreCase(response.get(1), "FETCH")) {
                            ...
                            if (uid != null) {
                                try {
                                    ...
                                    if (Up.DEBUG) {
                                        Log.v(Up.LOG_TAG, "Stored uid '" + uid + "' for msgSeq " + msgSeq + " into map " /*+ msgSeqUidMap.toString() */);
                                    }
                                } catch (Exception e) {
                                    Log.e(Up.LOG_TAG, "Unable to store uid '" + uid + "' for msgSeq " + msgSeq);
                                }
                            }
                            ...
                            if (message == null) {
                                if (Up.DEBUG)
                                    Log.d(Up.LOG_TAG, "Do not have message in messageMap for UID " + uid + " for " + getLogId());
                ...
                do {
                    ...
                    if ((response.mTag == null) &&
                        ...
                        if (!message.getUid().equals(uid)) {
                            if (Up.DEBUG)
                                Log.d(Up.LOG_TAG, "Did not ask for UID " + uid + " for " + getLogId());
            ...
            if (fetchList.containsKey("BODYSTRUCTURE")) {
                ...
                if (bs != null) {
                    try {
                        ...
                        if (Up.DEBUG)
                            Log.d(Up.LOG_TAG, "Error handling message for " + getLogId(), e);
                    ...
                    if (bracketed.size() > 1) {
                        ...
                        if (keyObj instanceof String) {
                            ...
                            if ("UIDNEXT".equalsIgnoreCase(key)) {
                                ...
                                if (Up.DEBUG)
                                    Log.d(Up.LOG_TAG, "Got UidNext = " + uidNext + " for " + getLogId());
                            }
                        }
                    }
        ...
        protected void handleUntaggedResponse(ImapResponse response) {
            if (response.mTag == null && response.size() > 1) {
                if (ImapResponseParser.equalsIgnoreCase(response.get(1), "EXISTS")) {
                    ...
                    if (Up.DEBUG)
                        Log.d(Up.LOG_TAG, "Got untagged EXISTS with value " + mMessageCount + " for " + getLogId());
                }
                ...
                if (ImapResponseParser.equalsIgnoreCase(response.get(1), "EXPUNGE") && mMessageCount > 0) {
                    mMessageCount--;
                    if (Up.DEBUG)
                        Log.d(Up.LOG_TAG, "Got untagged EXPUNGE with mMessageCount " + mMessageCount + " for " + getLogId());
                }
//            if (response.size() > 1) {
//                Object bracketedObj = response.get(1);
//                if (bracketedObj instanceof ImapList)
...
//                    [MESSAGE, RFC822, [NAME, Fwd: [#HTR-517941]:  update plans at 1am Friday - Memory allocation - displayware.eml], NIL, NIL, 7BIT, 5974, NIL, [INLINE, [FILENAME*0, Fwd: [#HTR-517941]:  update plans at 1am Friday - Memory all, FILENAME*1, ocation - displayware.eml]], NIL]
        ...
        public Map<String, String> appendMessages(Message[] messages) throws MessagingException {
            ...
            try {
                ...
                for (Message message : messages) {
                    ...
                    if (Up.DEBUG) {
                        Log.d(Up.LOG_TAG, "Got UID " + newUid + " for message for " + getLogId());
                    }
            ...
            try {
                ...
                */
                ...
                if (messageIdHeader == null || messageIdHeader.length == 0) {
                    if (Up.DEBUG)
                        Log.d(Up.LOG_TAG, "Did not get a message-id in order to search for UID  for " + getLogId());
                    return null;
                }
                String messageId = messageIdHeader[0];
                if (Up.DEBUG)
                    Log.d(Up.LOG_TAG, "Looking for UID for message with message-id " + messageId + " for " + getLogId());
        ...
        public String getNewPushState(String oldPushStateS, Message message) {
            try {
                ...
                Log.e(Up.LOG_TAG, "Exception while updated push state for " + getLogId(), e);
                return null;
            }
        }
        ...
        private MessagingException ioExceptionHandler(ImapConnection connection, IOException ioe) {
            Log.e(Up.LOG_TAG, "IOException for " + getLogId(), ioe);
            if (connection != null) {
                connection.close();
            }
            close();
        ...
        private List<ImapResponse> receiveCapabilities(List<ImapResponse> responses) {
            for (ImapResponse response : responses) {
                ...
                if (capabilityList != null && !capabilityList.isEmpty() &&
                    ...
                    if (Up.DEBUG) {
                        Log.d(Up.LOG_TAG, "Saving " + capabilityList.size() + " capabilities for " + getLogId());
                    }
...
//                            }
        ...
        public void open() throws IOException, MessagingException {
            ...
            try {
                ...
                Log.w(Up.LOG_TAG, "Could not set DNS ttl to 0 for " + getLogId(), e);
            }
            ...
            try {
                ...
                Log.w(Up.LOG_TAG, "Could not set DNS negative ttl to 0 for " + getLogId(), e);
            }
            ...
            try {
                ...
                for (int i = 0; i < addresses.length; i++) {
                    try {
                        if (Up.DEBUG && Up.DEBUG_PROTOCOL_IMAP) {
                            Log.d(Up.LOG_TAG, "Connecting to " + mSettings.getHost() + " as " +
                                    addresses[i]);
                        }
                ...
                if (Up.DEBUG && Up.DEBUG_PROTOCOL_IMAP)
                    Log.v(Up.LOG_TAG, getLogId() + "<<<" + nullResponse);
                ...
                if (!hasCapability(CAPABILITY_CAPABILITY)) {
                    if (Up.DEBUG)
                        Log.i(Up.LOG_TAG, "Did not get capabilities in banner, requesting CAPABILITY for " + getLogId());
                ...
                if (mSettings.getConnectionSecurity() == ConnectionSecurity.STARTTLS_REQUIRED) {
                    ...
                    if (hasCapability("STARTTLS")) {
                        ...
                        if (Up.DEBUG)
                            Log.i(Up.LOG_TAG, "Updating capabilities after STARTTLS for " + getLogId());
                        capabilities.clear();
                        List<ImapResponse> responses = receiveCapabilities(executeSimpleCommand(COMMAND_CAPABILITY));
                        if (responses.size() != 2) {
                            throw new MessagingException("Invalid CAPABILITY response received");
                ...
                authSuccess = true;
                if (Up.DEBUG) {
                    Log.d(Up.LOG_TAG, CAPABILITY_COMPRESS_DEFLATE + " = " + hasCapability(CAPABILITY_COMPRESS_DEFLATE));
                }
                if (hasCapability(CAPABILITY_COMPRESS_DEFLATE)) {
                    ConnectivityManager connectivityManager = (ConnectivityManager)Up.app.getSystemService(Context.CONNECTIVITY_SERVICE);
                    ...
                    if (netInfo != null) {
                        ...
                        if (Up.DEBUG)
                            Log.d(Up.LOG_TAG, "On network type " + type);
                        useCompression = mSettings.useCompression(type);

                    }
                    if (Up.DEBUG)
                        Log.d(Up.LOG_TAG, "useCompression " + useCompression);
                    if (useCompression) {
                        try {
                            ...
                            if (Up.DEBUG) {
                                Log.i(Up.LOG_TAG, "Compression enabled for " + getLogId());
                            }
                        } catch (Exception e) {
                            Log.e(Up.LOG_TAG, "Unable to negotiate compression", e);
                        }
                    }
                }
                ...
                if (Up.DEBUG)
                    Log.d(Up.LOG_TAG, "NAMESPACE = " + hasCapability(CAPABILITY_NAMESPACE)
                          + ", mPathPrefix = " + mSettings.getPathPrefix());

                if (mSettings.getPathPrefix() == null) {
                    if (hasCapability(CAPABILITY_NAMESPACE)) {
                        if (Up.DEBUG)
                            Log.i(Up.LOG_TAG, "mPathPrefix is unset and server has NAMESPACE capability");
                        ...
                        for (ImapResponse response : namespaceResponses) {
                            if (ImapResponseParser.equalsIgnoreCase(response.get(0), COMMAND_NAMESPACE)) {
                                if (Up.DEBUG)
                                    Log.d(Up.LOG_TAG, "Got NAMESPACE response " + response + " on " + getLogId());

                                Object personalNamespaces = response.get(1);
                                if (personalNamespaces != null && personalNamespaces instanceof ImapList) {
                                    if (Up.DEBUG)
                                        Log.d(Up.LOG_TAG, "Got personal namespaces: " + personalNamespaces);
                                    ...
                                    if (firstNamespace != null && firstNamespace instanceof ImapList) {
                                        if (Up.DEBUG)
                                            Log.d(Up.LOG_TAG, "Got first personal namespaces: " + firstNamespace);
                                        ...
                                        if (Up.DEBUG)
                                            Log.d(Up.LOG_TAG, "Got path '" + mSettings.getPathPrefix() + "' and separator '" + mSettings.getPathDelimeter() + "'");
                                    }
                                }
                            }
                        }
                        ...
                        if (Up.DEBUG)
                            Log.i(Up.LOG_TAG, "mPathPrefix is unset but server does not have NAMESPACE capability");
                    ...
                    try {
                        ...
                        for (ImapResponse response : nameResponses) {
                            if (ImapResponseParser.equalsIgnoreCase(response.get(0), "LIST")) {
                                ...
                                mSettings.setCombinedPrefix(null);
                                if (Up.DEBUG)
                                    Log.d(Up.LOG_TAG, "Got path delimeter '" + mSettings.getPathDelimeter() + "' for " + getLogId());
                            }
                        }
                    } catch (Exception e) {
                        Log.e(Up.LOG_TAG, "Unable to get path delimeter using LIST", e);
                    }
                }
                ...
                if (tokens != null && tokens.length > 1 && tokens[1] != null) {
                    Log.e(Up.LOG_TAG, "Stripping host/port from ConnectionException for " + getLogId(), ce);
                ...
                if (!authSuccess) {
                    Log.e(Up.LOG_TAG, "Failed to login, closing connection for " + getLogId());
                    close();
                }
            }
        }
            ...
            do {
                ...
                if (response.mTag != null) {
                    if (response.mTag.equalsIgnoreCase(tag)) {
                        ...
                        Log.w(Up.LOG_TAG, "After sending tag " + tag
        ...
        protected ArrayList<ImapResponse> readStatusResponse(String tag,
            ...
            do {
                ...
                if (Up.DEBUG && Up.DEBUG_PROTOCOL_IMAP)
                    Log.v(Up.LOG_TAG, getLogId() + "<<<" + response);

                if (response.mTag != null && !response.mTag.equalsIgnoreCase(tag)) {
                    Log.w(Up.LOG_TAG, "After sending tag " + tag + ", got tag response from previous command " + response + " for " + getLogId());
        ...
        protected boolean isIdleCapable() {
            if (Up.DEBUG)
                Log.v(Up.LOG_TAG, "Connection " + getLogId() + " has " + capabilities.size() + " capabilities");
...
//            }
        ...
        public void sendContinuation(String continuation) throws IOException {
            ...
            if (Up.DEBUG && Up.DEBUG_PROTOCOL_IMAP)
                Log.v(Up.LOG_TAG, getLogId() + ">>> " + continuation);
        ...
        throws MessagingException, IOException {
            try {
                ...
                if (Up.DEBUG && Up.DEBUG_PROTOCOL_IMAP) {
                    if (sensitive && !Up.DEBUG_SENSITIVE) {
                        Log.v(Up.LOG_TAG, getLogId() + ">>> "
                              + "[Command Hidden, Enable Sensitive Debug Logging To Show]");
                    } else {
                        Log.v(Up.LOG_TAG, getLogId() + ">>> " + commandToSend);
                    }
                }
    ...
    public class ImapFolderPusher extends ImapFolder implements UntaggedHandler {
        ...
        public void refresh() throws IOException, MessagingException {
            if (idling.get()) {
                wakeLock.acquire(Up.PUSH_WAKE_LOCK_TIMEOUT);
                sendDone();
            }
        }
        ...
        public void start() {
            Runnable runner = new Runnable() {
                public void run() {
                    wakeLock.acquire(Up.PUSH_WAKE_LOCK_TIMEOUT);
                    if (Up.DEBUG)
                        Log.i(Up.LOG_TAG, "Pusher starting for " + getLogId());
                    ...
                    while (!stop.get()) {
                        try {
                            ...
                            try {
                                ...
                                if (Up.DEBUG)
                                    Log.i(Up.LOG_TAG, "Got oldUidNext " + oldUidNext + " for " + getLogId());
                            } catch (Exception e) {
                                Log.e(Up.LOG_TAG, "Unable to get oldUidNext for " + getLogId(), e);
                            }
                            ...
                            if (newUidNext == -1) {
                                if (Up.DEBUG) {
                                    Log.d(Up.LOG_TAG, "uidNext is -1, using search to find highest UID");
                                }
                                long highestUid = getHighestUid();
                                if (highestUid != -1L) {
                                    if (Up.DEBUG)
                                        Log.d(Up.LOG_TAG, "highest UID = " + highestUid);
                            ...
                            if (newUidNext > startUid) {
                                ...
                                if (Up.DEBUG)
                                    Log.i(Up.LOG_TAG, "Needs sync from uid " + startUid  + " to " + newUidNext + " for " + getLogId());
                                ...
                                while (!storedUntaggedResponses.isEmpty()) {
                                    if (Up.DEBUG)
                                        Log.i(Up.LOG_TAG, "Processing " + storedUntaggedResponses.size() + " untagged responses from previous commands for " + getLogId());
                                ...
                                if (Up.DEBUG)
                                    Log.i(Up.LOG_TAG, "About to IDLE for " + getLogId());
                            ...
                            try {
                                ...
                                Log.e(Up.LOG_TAG, "Got exception while closing for exception for " + getLogId(), me);
                            }
                            if (stop.get()) {
                                Log.i(Up.LOG_TAG, "Got exception while idling, but stop is set for " + getLogId());
                            } else {
                                receiver.pushError("Push error for " + getName(), e);
                                Log.e(Up.LOG_TAG, "Got exception while idling for " + getLogId(), e);
                                ...
                                if (idleFailureCount.incrementAndGet() > IDLE_FAILURE_COUNT_LIMIT) {
                                    Log.e(Up.LOG_TAG, "Disabling pusher for " + getLogId() + " after " + idleFailureCount.get() + " consecutive errors");
                    ...
                    try {
                        if (Up.DEBUG)
                            Log.i(Up.LOG_TAG, "Pusher for " + getLogId() + " is exiting");
                        close();
                    } catch (Exception me) {
                        Log.e(Up.LOG_TAG, "Got exception while closing for " + getLogId(), me);
                    } finally {
                        wakeLock.release();
                    }
                }
        ...
        protected void handleUntaggedResponse(ImapResponse response) {
            if (response.mTag == null && response.size() > 1) {
                ...
                if (ImapResponseParser.equalsIgnoreCase(responseType, "FETCH")
                    ...
                    if (Up.DEBUG)
                        Log.d(Up.LOG_TAG, "Storing response " + response + " for later processing");
        ...
        protected void processUntaggedResponses(List<ImapResponse> responses) throws MessagingException {
            ...
            if (Up.DEBUG)
                Log.d(Up.LOG_TAG, "UIDs for messages needing flag sync are " + flagSyncMsgSeqs + "  for " + getLogId());
        ...
        private void syncMessages(int end, boolean newArrivals) throws MessagingException {
            ...
            if (messageArray != null && messageArray.length > 0) {
                ...
                if (Up.DEBUG)
                    Log.i(Up.LOG_TAG, "Got newUid " + newUid + " for message " + end + " on " + getLogId());
                long startUid = oldUidNext;
                ...
                if (newUid >= startUid) {
                    ...
                    if (Up.DEBUG)
                        Log.i(Up.LOG_TAG, "Needs sync from uid " + startUid  + " to " + newUid + " for " + getLogId());
                    List<Message> messages = new ArrayList<Message>();
        ...
        private void removeMessages(List<String> removeUids) {
            ...
            try {
                ...
                for (Message existingMessage : existingMessages) {
                    ...
                    Log.w(Up.LOG_TAG, "Message with UID " + existingUid + " still exists on server, not expunging");
                ...
                for (String uid : removeUids) {
                    ...
                    try {
                        ...
                        Log.e(Up.LOG_TAG, "Unable to set DELETED flag on message " + message.getUid());
                    }
                    messages.add(message);
                }
                receiver.messagesRemoved(this, messages);
            } catch (Exception e) {
                Log.e(Up.LOG_TAG, "Cannot remove EXPUNGEd messages", e);
            }
        ...
        protected int processUntaggedResponse(long oldMessageCount, ImapResponse response, List<Long> flagSyncMsgSeqs, List<String> removeMsgUids) {
            ...
            if (response.mTag == null && response.size() > 1) {
                try {
                    Object responseType = response.get(1);
                    if (ImapResponseParser.equalsIgnoreCase(responseType, "FETCH")) {
                        Log.i(Up.LOG_TAG, "Got FETCH " + response);
                        long msgSeq = response.getLong(0);

                        if (Up.DEBUG)
                            Log.d(Up.LOG_TAG, "Got untagged FETCH for msgseq " + msgSeq + " for " + getLogId());
                    ...
                    if (ImapResponseParser.equalsIgnoreCase(responseType, "EXPUNGE")) {
                        ...
                        if (Up.DEBUG)
                            Log.d(Up.LOG_TAG, "Got untagged EXPUNGE for msgseq " + msgSeq + " for " + getLogId());
                        ...
                        for (long msgSeqNum : msgSeqs) {
                            if (Up.DEBUG) {
                                Log.v(Up.LOG_TAG, "Comparing EXPUNGEd msgSeq " + msgSeq + " to " + msgSeqNum);
                            }
                            if (msgSeqNum == msgSeq) {
                                String uid = msgSeqUidMap.get(msgSeqNum);
                                if (Up.DEBUG) {
                                    Log.d(Up.LOG_TAG, "Scheduling removal of UID " + uid + " because msgSeq " + msgSeqNum + " was expunged");
                                }
                                ...
                                String uid = msgSeqUidMap.get(msgSeqNum);
                                if (Up.DEBUG) {
                                    Log.d(Up.LOG_TAG, "Reducing msgSeq for UID " + uid + " from " + msgSeqNum + " to " + (msgSeqNum - 1));
                                }
                                msgSeqUidMap.remove(msgSeqNum);
                    ...
                    Log.e(Up.LOG_TAG, "Could not handle untagged FETCH for " + getLogId(), e);
                }
            }
        ...
        public void stop() {
            ...
            if (conn != null) {
                if (Up.DEBUG)
                    Log.v(Up.LOG_TAG, "Closing mConnection to stop pushing for " + getLogId());
                conn.close();
            } else {
                Log.w(Up.LOG_TAG, "Attempt to interrupt null mConnection to stop pushing on folderPusher for " + getLogId());
            }
        }

        public void handleAsyncUntaggedResponse(ImapResponse response) {
            if (Up.DEBUG)
                Log.v(Up.LOG_TAG, "Got async response: " + response);

            if (stop.get()) {
                if (Up.DEBUG)
                    Log.d(Up.LOG_TAG, "Got async untagged response: " + response + ", but stop is set for " + getLogId());

                try {
                    sendDone();
                } catch (Exception e) {
                    Log.e(Up.LOG_TAG, "Exception while sending DONE for " + getLogId(), e);
                }
            } else {
                if (response.mTag == null) {
                    if (response.size() > 1) {
                        ...
                        if (ImapResponseParser.equalsIgnoreCase(responseType, "EXISTS") || ImapResponseParser.equalsIgnoreCase(responseType, "EXPUNGE") ||
                            ...
                            if (Up.DEBUG)
                                Log.d(Up.LOG_TAG, "Got useful async untagged response: " + response + " for " + getLogId());
                        ...
                        if (Up.DEBUG)
                            Log.d(Up.LOG_TAG, "Idling " + getLogId());
    ...
    public class ImapPusher implements Pusher {
            ...
            synchronized (folderPushers) {
                for (ImapFolderPusher folderPusher : folderPushers.values()) {
                    try {
                        ...
                        Log.e(Up.LOG_TAG, "Got exception while refreshing for " + folderPusher.getName(), e);
                    }
                }
            }
        }
        ...
        public void stop() {
            if (Up.DEBUG)
                Log.i(Up.LOG_TAG, "Requested stop of IMAP pusher");

            synchronized (folderPushers) {
                for (ImapFolderPusher folderPusher : folderPushers.values()) {
                    try {
                        if (Up.DEBUG)
                            Log.i(Up.LOG_TAG, "Requesting stop of IMAP folderPusher " + folderPusher.getName());
                        folderPusher.stop();
                    } catch (Exception e) {
                        Log.e(Up.LOG_TAG, "Got exception while stopping " + folderPusher.getName(), e);
                    }
                }
                folderPushers.clear();
            }
                    ...
                    if (thisState.hasMoreTokens()) {
                        ...
                        if ("uidNext".equalsIgnoreCase(key) && thisState.hasMoreTokens()) {
                            ...
                            try {
                                ...
                                Log.e(Up.LOG_TAG, "Unable to part uidNext value " + value, e);
                            }
