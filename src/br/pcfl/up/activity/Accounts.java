...
import br.pcfl.up.activity.misc.ExtendedAsyncTask;
import br.pcfl.up.activity.misc.NonConfigurationInstance;
import br.pcfl.up.activity.setup.AccountSettings;
import br.pcfl.up.activity.setup.AccountSetupBasics;
import br.pcfl.up.activity.setup.Prefs;
import br.pcfl.up.activity.setup.WelcomeMessage;
import br.pcfl.up.controller.MessagingController;
import br.pcfl.up.helper.SizeFormatter;
...
import br.pcfl.up.preferences.SettingsExporter;
import br.pcfl.up.preferences.SettingsImportExportException;
import br.pcfl.up.preferences.SettingsImporter;
import br.pcfl.up.preferences.SettingsImporter.AccountDescription;
import br.pcfl.up.preferences.SettingsImporter.AccountDescriptionPair;
import br.pcfl.up.preferences.SettingsImporter.ImportContents;
import br.pcfl.up.preferences.SettingsImporter.ImportResults;
...
import br.pcfl.up.view.ColorChip;

import de.cketti.library.changelog.ChangeLog;


public class Accounts extends UpListActivity implements OnItemClickListener {
        ...
        public void accountSizeChanged(final Account account, final long oldSize, final long newSize) {
            runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    if (newSize != -1 && stats != null && Up.measureAccounts()) {
                        stats.size = newSize;
                    }
    ...
    ActivityListener mListener = new ActivityListener() {
        ...
        public void folderStatusChanged(Account account, String folderName, int unreadMessageCount) {
            try {
                ...
                if (stats == null) {
                    Log.w(Up.LOG_TAG, "Unable to get account stats");
                } else {
                    accountStatusChanged(account, stats);
                }
            } catch (Exception e) {
                Log.e(Up.LOG_TAG, "Unable to get account stats", e);
            }
        }
        @Override
    ...
    public void onNewIntent(Intent intent) {
        ...
        Log.i(Up.LOG_TAG, "Accounts Activity got uri " + uri);
        if (uri != null) {
            ContentResolver contentResolver = getContentResolver();

            Log.i(Up.LOG_TAG, "Accounts Activity got content of type " + contentResolver.getType(uri));

            String contentType = contentResolver.getType(uri);
            if (MimeUtility.Up_SETTINGS_MIME_TYPE.equals(contentType)) {
                onImport(uri);
            }
        }
    }
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (startup && Up.startIntegratedInbox() && !Up.isHideSpecialAccounts()) {
...
//        }
        ...
        if (!Up.isHideSpecialAccounts() && accounts.length > 0) {
        ...
        for (BaseAccount account : newAccounts) {
            ...
            } else if (Up.countSearchMessages() && account instanceof SearchAccount) {
    ...
    private boolean onOpenAccount(BaseAccount account) {
        if (account instanceof SearchAccount) {
            ...
            if (!realAccount.isEnabled()) {
                ...
                Log.i(Up.LOG_TAG, "refusing to open account that is not available");
                return false;
            }
            ...
            new Runnable() {
                ...
                public void run() {
                    if (mSelectedContextAccount instanceof Account) {
                        ...
                        Up.setServicesEnabled(Accounts.this);
                        refresh();
                    }
                }
            });
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        Log.i(Up.LOG_TAG, "onActivityResult requestCode = " + requestCode + ", resultCode = " + resultCode + ", data = " + data);
    ...
    class AccountsAdapter extends ArrayAdapter<BaseAccount> {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (stats != null) {
                ...
                holder.flaggedMessageCountWrapper.setVisibility(Up.messageListStars() && stats.flaggedMessageCount > 0 ? View.VISIBLE : View.GONE);
    ...
    private static class ExportAsyncTask extends ExtendedAsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            try {
                ...
                Log.w(Up.LOG_TAG, "Exception during export", e);
                return false;
            }
            return true;
        }
    ...
    private static class ImportAsyncTask extends ExtendedAsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            try {
                ...
                Log.w(Up.LOG_TAG, "Exception during import", e);
                return false;
            } catch (FileNotFoundException e) {
                Log.w(Up.LOG_TAG, "Couldn't open import file", e);
                return false;
            } catch (Exception e) {
                Log.w(Up.LOG_TAG, "Unknown error", e);
                return false;
            }
            return true;
        }
    ...
    private static class ListImportContentsAsyncTask extends ExtendedAsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            try {
                ...
                Log.w(Up.LOG_TAG, "Couldn't read content from URI " + mUri);
                return false;
            }
            return true;
        }
