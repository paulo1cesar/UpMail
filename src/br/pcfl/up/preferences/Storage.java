...
public class Storage implements SharedPreferences {
    ...
    private SQLiteDatabase openDB() {
        ...
        if (mDb.getVersion() == 1) {
            Log.i(Up.LOG_TAG, "Updating preferences to urlencoded username/password");
        ...
        if (mDb.getVersion() != DB_VERSION) {
            Log.i(Up.LOG_TAG, "Creating Storage database");
    ...
    public static Storage getStorage(Context context) {
        ...
        if (tmpStorage != null) {
            if (Up.DEBUG) {
                Log.d(Up.LOG_TAG, "Returning already existing Storage");
            }
            return tmpStorage;
        } else {
            if (Up.DEBUG) {
                Log.d(Up.LOG_TAG, "Creating provisional storage");
            }
            tmpStorage = new Storage(context);
            Storage oldStorage = storages.putIfAbsent(context, tmpStorage);
            if (oldStorage != null) {
                if (Up.DEBUG) {
                    Log.d(Up.LOG_TAG, "Another thread beat us to creating the Storage, returning that one");
                }
                return oldStorage;
            } else {
                if (Up.DEBUG) {
                    Log.d(Up.LOG_TAG, "Returning the Storage we created");
                }
                return tmpStorage;
            }
        }
    ...
    private void loadValues() {
        ...
        Log.i(Up.LOG_TAG, "Loading preferences from DB into Storage");
        Cursor cursor = null;
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                if (Up.DEBUG) {
                    Log.d(Up.LOG_TAG, "Loading key '" + key + "', value = '" + value + "'");
                }
            ...
            Log.i(Up.LOG_TAG, "Preferences load took " + (endTime - startTime) + "ms");
        }
    }
    ...
    //@Override
    public br.pcfl.up.preferences.Editor edit() {
        return new br.pcfl.up.preferences.Editor(this);
    }
    ...
    public float getFloat(String key, float defValue) {
        ...
        try {
            ...
            Log.e(Up.LOG_TAG, "Could not parse float", nfe);
            return defValue;
        }
    }
    ...
    public int getInt(String key, int defValue) {
        ...
        try {
            ...
            Log.e(Up.LOG_TAG, "Could not parse int", nfe);
            return defValue;
        }
    }
    ...
    public long getLong(String key, long defValue) {
        ...
        try {
            ...
            Log.e(Up.LOG_TAG, "Could not parse long", nfe);
            return defValue;
        }
    }
    ...
    private void writeValue(SQLiteDatabase mDb, String key, String value) {
        ...
        if (result == -1) {
            Log.e(Up.LOG_TAG, "Error writing key '" + key + "', value = '" + value + "'");
        }
    }
