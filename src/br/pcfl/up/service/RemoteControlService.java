...
import br.pcfl.up.Up.BACKGROUND_OPS;
...
public class RemoteControlService extends CoreService {
    private final static String RESCHEDULE_ACTION = "br.pcfl.up.service.RemoteControlService.RESCHEDULE_ACTION";
    private final static String PUSH_RESTART_ACTION = "br.pcfl.up.service.RemoteControlService.PUSH_RESTART_ACTION";

    private final static String SET_ACTION = "br.pcfl.up.service.RemoteControlService.SET_ACTION";
    ...
    public int startService(final Intent intent, final int startId) {
        if (Up.DEBUG)
            Log.i(Up.LOG_TAG, "RemoteControlService started with startId = " + startId);
        final Preferences preferences = Preferences.getPreferences(this);

        if (RESCHEDULE_ACTION.equals(intent.getAction())) {
            if (Up.DEBUG)
                Log.i(Up.LOG_TAG, "RemoteControlService requesting MailService poll reschedule");
            MailService.actionReschedulePoll(this, null);
        }
        if (PUSH_RESTART_ACTION.equals(intent.getAction())) {
            if (Up.DEBUG)
                Log.i(Up.LOG_TAG, "RemoteControlService requesting MailService push restart");
            MailService.actionRestartPushers(this, null);
        } else if (RemoteControlService.SET_ACTION.equals(intent.getAction())) {
            if (Up.DEBUG)
                Log.i(Up.LOG_TAG, "RemoteControlService got request to change settings");
            execute(getApplication(), new Runnable() {
                public void run() {
                    try {
                        ...
                        if (Up.DEBUG) {
                            if (allAccounts) {
                                Log.i(Up.LOG_TAG, "RemoteControlService changing settings for all accounts");
                            } else {
                                Log.i(Up.LOG_TAG, "RemoteControlService changing settings for account with UUID " + uuid);
                            }
                        }
                        ...
                        for (Account account : accounts) {
                            ...
                            if (allAccounts || account.getUuid().equals(uuid)) {
                                ...
                                if (Up.DEBUG)
                                    Log.i(Up.LOG_TAG, "RemoteControlService changing settings for account " + account.getDescription());
                        ...
                        if (Up.DEBUG)
                            Log.i(Up.LOG_TAG, "RemoteControlService changing global settings");

                        String backgroundOps = intent.getStringExtra(K9_BACKGROUND_OPERATIONS);
                        if (UpRemoteControl.K9_BACKGROUND_OPERATIONS_ALWAYS.equals(backgroundOps)
                                || UpRemoteControl.K9_BACKGROUND_OPERATIONS_NEVER.equals(backgroundOps)
                                || UpRemoteControl.K9_BACKGROUND_OPERATIONS_WHEN_CHECKED.equals(backgroundOps)
                        || UpRemoteControl.K9_BACKGROUND_OPERATIONS_WHEN_CHECKED_AUTO_SYNC.equals(backgroundOps)) {
                            BACKGROUND_OPS newBackgroundOps = BACKGROUND_OPS.valueOf(backgroundOps);
                            boolean needsReset = Up.setBackgroundOps(newBackgroundOps);
                        ...
                        if (theme != null) {
                            Up.setUpTheme(UpRemoteControl.K9_THEME_DARK.equals(theme) ? Up.Theme.DARK : Up.Theme.LIGHT);
                        }
                        ...
                        if (needsReschedule) {
                            Intent i = new Intent();
                            i.setClassName(getApplication().getPackageName(), "br.pcfl.up.service.RemoteControlService");
                        ...
                        Log.e(Up.LOG_TAG, "Could not handle K9_SET", e);
