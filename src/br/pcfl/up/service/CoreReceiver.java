...
public class CoreReceiver extends BroadcastReceiver {
    ...
    private static Integer getWakeLock(Context context) {
        ...
        wakeLock.acquire(Up.BOOT_RECEIVER_WAKE_LOCK_TIMEOUT);
        Integer tmpWakeLockId = wakeLockSeq.getAndIncrement();
        wakeLocks.put(tmpWakeLockId, wakeLock);
        if (Up.DEBUG)
            Log.v(Up.LOG_TAG, "CoreReceiver Created wakeLock " + tmpWakeLockId);
    ...
    private static void releaseWakeLock(Integer wakeLockId) {
        if (wakeLockId != null) {
            ...
            if (wl != null) {
                if (Up.DEBUG)
                    Log.v(Up.LOG_TAG, "CoreReceiver Releasing wakeLock " + wakeLockId);
                wl.release();
            } else {
                Log.w(Up.LOG_TAG, "BootReceiver WakeLock " + wakeLockId + " doesn't exist");
            }
        }
    }
    ...
    public static void releaseWakeLock(Context context, int wakeLockId) {
        if (Up.DEBUG)
            Log.v(Up.LOG_TAG, "CoreReceiver Got request to release wakeLock " + wakeLockId);
        Intent i = new Intent();
