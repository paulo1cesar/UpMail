...
import br.pcfl.up.Account.FolderMode;
...
public class MailService extends CoreService {
    ...
    public void onCreate() {
        super.onCreate();
        if (Up.DEBUG)
            Log.v(Up.LOG_TAG, "***** MailService *****: onCreate");
    }
    ...
    public int startService(Intent intent, int startId) {
        ...
        Up.BACKGROUND_OPS bOps = Up.getBackgroundOps();
        ...
        if (Up.DEBUG)
            Log.i(Up.LOG_TAG, "MailService.onStart(" + intent + ", " + startId
                  + "), hasConnectivity = " + hasConnectivity + ", doBackground = " + doBackground);

        // MessagingController.getInstance(getApplication()).addListener(mListener);
        if (ACTION_CHECK_MAIL.equals(intent.getAction())) {
            if (Up.DEBUG)
                Log.i(Up.LOG_TAG, "***** MailService *****: checking mail");
            ...
            rescheduleAllInBackground(hasConnectivity, doBackground, startId);
            if (Up.DEBUG)
                Log.i(Up.LOG_TAG, "Got connectivity action with hasConnectivity = " + hasConnectivity + ", doBackground = " + doBackground);
        ...
        if (Up.DEBUG)
            Log.i(Up.LOG_TAG, "MailService.onStart took " + (System.currentTimeMillis() - startTime) + "ms");
    ...
    public void onDestroy() {
        if (Up.DEBUG)
            Log.v(Up.LOG_TAG, "***** MailService *****: onDestroy()");
        super.onDestroy();
    ...
    public static void saveLastCheckEnd(Context context) {
        ...
        if (Up.DEBUG)
            Log.i(Up.LOG_TAG, "Saving lastCheckEnd = " + new Date(lastCheckEnd));
    ...
    private void rescheduleAllInBackground(final boolean hasConnectivity,
        ...
        }, Up.MAIL_SERVICE_WAKE_LOCK_TIMEOUT, startId);
    }
    ...
    private void reschedulePoll(final boolean hasConnectivity, final boolean doBackground,
        ...
        if (!(hasConnectivity && doBackground)) {
            if (Up.DEBUG) {
                Log.i(Up.LOG_TAG, "No connectivity, canceling check for " +
        ...
        if (lastCheckEnd > System.currentTimeMillis()) {
            Log.i(Up.LOG_TAG, "The database claims that the last time mail was checked was in " +
        ...
        if (shortestInterval == -1) {
            if (Up.DEBUG) {
                Log.i(Up.LOG_TAG, "No next check scheduled for package " +
                        getApplication().getPackageName());
            }
            ...
            if (Up.DEBUG) {
                Log.i(Up.LOG_TAG, "previousInterval = " + previousInterval +
            ...
            try {
                if (Up.DEBUG) {
                    Log.i(Up.LOG_TAG, "Next check for package " +
                            getApplication().getPackageName() + " scheduled for " +
                            new Date(nextTime));
                }
            } catch (Exception e) {
                // I once got a NullPointerException deep in new Date();
                Log.e(Up.LOG_TAG, "Exception while logging", e);
            }
    ...
    private void reschedulePushers(boolean hasConnectivity, boolean doBackground) {
        if (Up.DEBUG) {
            Log.i(Up.LOG_TAG, "Rescheduling pushers");
        }
        ...
        if (!(hasConnectivity && doBackground)) {
            if (Up.DEBUG) {
                Log.i(Up.LOG_TAG, "Not scheduling pushers:  connectivity? " + hasConnectivity +
    ...
    private void setupPushers() {
        ...
        for (Account account : Preferences.getPreferences(MailService.this).getAccounts()) {
            if (Up.DEBUG)
                Log.i(Up.LOG_TAG, "Setting up pushers for account " + account.getDescription());
    ...
    private void refreshPushers() {
        try {
            ...
            if (Up.DEBUG)
                Log.i(Up.LOG_TAG, "Refreshing pushers");
            ...
            for (Pusher pusher : pushers) {
                ...
                if (sinceLast + 10000 > refreshInterval) { // Add 10 seconds to keep pushers in sync, avoid drift
                    if (Up.DEBUG) {
                        Log.d(Up.LOG_TAG, "PUSHREFRESH: refreshing lastRefresh = " + lastRefresh + ", interval = " + refreshInterval
                    ...
                    if (Up.DEBUG) {
                        Log.d(Up.LOG_TAG, "PUSHREFRESH: NOT refreshing lastRefresh = " + lastRefresh + ", interval = " + refreshInterval
                              + ", nowTime = " + nowTime + ", sinceLast = " + sinceLast);
                    }
                }
            }
            ...
            if (Up.DEBUG) {
                Log.d(Up.LOG_TAG, "PUSHREFRESH:  trying to send mail in all folders!");
            }

            MessagingController.getInstance(getApplication()).sendPendingMessages(null);

        } catch (Exception e) {
            Log.e(Up.LOG_TAG, "Exception while refreshing pushers", e);
        }
    }
    ...
    private void schedulePushers() {
        ...
        if (Up.DEBUG) {
            Log.v(Up.LOG_TAG, "Pusher refresh interval = " + minInterval);
        }
        if (minInterval > 0) {
            long nextTime = System.currentTimeMillis() + minInterval;
            if (Up.DEBUG)
                Log.d(Up.LOG_TAG, "Next pusher refresh scheduled for " + new Date(nextTime));
            Intent i = new Intent();
            i.setClassName(getApplication().getPackageName(), "com.fsck.k9.service.MailService");
