...
public class SleepService extends CoreService {
    ...
    public static void sleep(Context context, long sleepTime, TracingWakeLock wakeLock, long wakeLockTimeout) {
        ...
        if (Up.DEBUG)
            Log.d(Up.LOG_TAG, "SleepService Preparing CountDownLatch with id = " + id + ", thread " + Thread.currentThread().getName());
        ...
        try {
            ...
            if (!countedDown) {
                if (Up.DEBUG)
                    Log.d(Up.LOG_TAG, "SleepService latch timed out for id = " + id + ", thread " + Thread.currentThread().getName());
            }
        } catch (InterruptedException ie) {
            Log.e(Up.LOG_TAG, "SleepService Interrupted while awaiting latch", ie);
        }
        SleepDatum releaseDatum = sleepData.remove(id);
        if (releaseDatum == null) {
            try {
                if (Up.DEBUG)
                    Log.d(Up.LOG_TAG, "SleepService waiting for reacquireLatch for id = " + id + ", thread " + Thread.currentThread().getName());
                if (!sleepDatum.reacquireLatch.await(5000, TimeUnit.MILLISECONDS)) {
                    Log.w(Up.LOG_TAG, "SleepService reacquireLatch timed out for id = " + id + ", thread " + Thread.currentThread().getName());
                } else if (Up.DEBUG)
                    Log.d(Up.LOG_TAG, "SleepService reacquireLatch finished for id = " + id + ", thread " + Thread.currentThread().getName());
            } catch (InterruptedException ie) {
                Log.e(Up.LOG_TAG, "SleepService Interrupted while awaiting reacquireLatch", ie);
            }
        } else {
        ...
        if (actualSleep < sleepTime) {
            Log.w(Up.LOG_TAG, "SleepService sleep time too short: requested was " + sleepTime + ", actual was " + actualSleep);
        } else {
            if (Up.DEBUG)
                Log.d(Up.LOG_TAG, "SleepService requested sleep time was " + sleepTime + ", actual was " + actualSleep);
        }
    }
    ...
    private static void endSleep(Integer id) {
        if (id != -1) {
            ...
            if (sleepDatum != null) {
                ...
                if (latch == null) {
                    Log.e(Up.LOG_TAG, "SleepService No CountDownLatch available with id = " + id);
                } else {
                    if (Up.DEBUG)
                        Log.d(Up.LOG_TAG, "SleepService Counting down CountDownLatch with id = " + id);
                    latch.countDown();
                }
                ...
                if (Up.DEBUG)
                    Log.d(Up.LOG_TAG, "SleepService Sleep for id " + id + " already finished");
            }
        }
    }
    ...
    private static void reacquireWakeLock(SleepDatum sleepDatum) {
        ...
        if (wakeLock != null) {
            synchronized (wakeLock) {
                ...
                if (Up.DEBUG)
                    Log.d(Up.LOG_TAG, "SleepService Acquiring wakeLock for " + timeout + "ms");
