...
public class StorageGoneReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if (Up.DEBUG) {
            Log.v(Up.LOG_TAG, "StorageGoneReceiver: " + intent.toString());
        }

        final String path = uri.getPath();

        if (Intent.ACTION_MEDIA_EJECT.equals(action)) {
            StorageManager.getInstance(Up.app).onBeforeUnmount(path);
        } else if (Intent.ACTION_MEDIA_UNMOUNTED.equals(action)) {
            StorageManager.getInstance(Up.app).onAfterUnmount(path);
        }
    }
