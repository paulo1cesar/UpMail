...
public abstract class CoreService extends Service {
    ...
    protected static void addWakeLock(Context context, Intent intent) {
        TracingWakeLock wakeLock = acquireWakeLock(context, "CoreService addWakeLock",
                Up.MAIL_SERVICE_WAKE_LOCK_TIMEOUT);
    ...
    public void onCreate() {
        if (Up.DEBUG) {
            Log.i(Up.LOG_TAG, "CoreService: " + className + ".onCreate()");
        }
    ...
    public final int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Up.DEBUG) {
            Log.i(Up.LOG_TAG, "CoreService: " + className + ".onStart(" + intent + ", " + startId + ")");
        }
        ...
        if (coreWakeLockId != -1) {
            if (Up.DEBUG) {
                Log.d(Up.LOG_TAG, "Got core wake lock id " + coreWakeLockId);
            }
            ...
            if (coreWakeLock != null) {
                if (Up.DEBUG) {
                    Log.d(Up.LOG_TAG, "Found core wake lock with id " + coreWakeLockId +
                            ", releasing");
                }
    ...
    public void execute(Context context, final Runnable runner, int wakeLockTime,
        ...
        Runnable myRunner = new Runnable() {
            public void run() {
                try {
                    ...
                    if (Up.DEBUG) {
                        Log.d(Up.LOG_TAG, "CoreService (" + className + ") running Runnable " +
                    ...
                    try {
                        if (Up.DEBUG) {
                            Log.d(Up.LOG_TAG, "CoreService (" + className + ") completed " +
        ...
        if (mThreadPool == null) {
            Log.e(Up.LOG_TAG, "CoreService.execute (" + className + ") called with no thread " +
            ...
            if (Up.DEBUG) {
                Log.d(Up.LOG_TAG, "CoreService (" + className + ") queueing Runnable " +
                        runner.hashCode() + " with startId " + startId);
            }
            ...
            try {
                ...
                Log.i(Up.LOG_TAG, "CoreService: " + className + " is shutting down, ignoring " +
    ...
    public void onLowMemory() {
        Log.w(Up.LOG_TAG, "CoreService: " + className + ".onLowMemory() - Running low on memory");
    }
    ...
    public void onDestroy() {
        if (Up.DEBUG) {
            Log.i(Up.LOG_TAG, "CoreService: " + className + ".onDestroy()");
        }
